...
**Then why does the boss's vision pass feel cheaper?** Because her *input* is smaller, not her *model*. The boss runs a big vision model on a downsampled image. Same model, smaller input, less per-pass compute. The worker runs a smaller vision model on a full-resolution slice. Different model, bigger input, also less per-pass compute (because the model is smaller). They end up with comparable per-pass costs — but the boss is the one processing the high-quality, low-data view, while the worker is processing the lower-quality, high-data view. The boss is not the constrained one. The worker is.

**Concretely, in the bunker swarm:** the RajaBee runs the biggest vision model the boss machine can host, on a downsampled gestalt view of the whole bunker map. She catches subtle things on a small picture. The DwarfQueens run somewhat smaller vision models on bigger sub-region views. The workers at the bottom run the smallest vision models the worker hardware can host, but on the highest-resolution slice of their tiny region. **Everyone is running the most capable model their box can afford. The boss happens to be doing it on smaller data because she is processing a downsampled summary view.**
