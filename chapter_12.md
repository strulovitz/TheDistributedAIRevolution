...
## Correction added after writing — the sound case, honestly

Everything above about the sound case is the shape of the right idea. Part of it, specifically the operation used to produce the "low-fidelity at the parent" copy for long audio, is wrong. The error survived first-draft review and was caught two weeks later during the first actual end-to-end test of the architecture on a real three-minute clip. Nir caught it. Claude, who helped write the original, missed it. The fix is also Nir's. I am writing this correction down as a permanent part of the chapter rather than silently editing the earlier paragraphs, because the reasoning behind the mistake — and the reasoning behind Nir's two independent saves — is more useful to the reader than a quiet patch would be.

**Where the original sound section breaks.** Image and sound look like mirror problems: two spatial axes versus one time axis, downsample on each to produce a low-fidelity copy of the whole, give it to the parent, done. The symmetry is false. When you halve an image's sample rate on both axes — from four thousand by four thousand down to two thousand by two thousand — you reduce the total data by a factor of four, and the model's fixed input window, which is already expressed in some small number of pixels (a vision model might accept a 1024×1024 input), now comfortably holds the whole scene at coarser spatial resolution. The whole image fits through the model in one pass. That is the thing the analogy depends on.

When you halve an audio recording's sample rate — from 44.1 kHz to 22.05 kHz — you reduce the data per second, but **you do not compress time**. The recording is still an hour long. And every local audio-understanding model that exists today has a receptive field of tens of seconds — whisper-large has a thirty-second sliding window it stitches internally, and audio-understanding models like Qwen2-Audio and Phi-4-multimodal-audio accept inputs in the tens-of-seconds to low-single-minutes range at most. No amount of sample-rate reduction, and no amount of bitrate reduction, lets those models hold an hour of audio in a single forward pass. Lowering fidelity reduces the weight per second; it does nothing to the axis the model actually struggles with, which is duration. The book's original sound section described a clean analogy to the image case that simply does not survive contact with real local-model capacity.

**Nir's first save.** After the first end-to-end test of the architecture ran the parent's "gestalt" pass on a three-minute audio clip and produced a partial output covering only the first minute or so, Nir said plainly: *"the book is totally wrong, making it low fidelity does not help at all, we are still limited by files that are playing time longer than a few seconds."* This was correct and was the first piece of the fix. Sound has one dimension and it is time; lowering the sample rate does nothing to that dimension; the receptive-field ceiling is real and it is measured in seconds, not in bytes.

**Nir's second save — the actual fix.** Nir then asked whether the gestalt could be recovered by doing to an audio recording what old movies do when you press fast-forward: play it faster. *This* is the true one-dimensional analogue of two-dimensional image downsampling, and it is what the book should have said from the beginning. A three-minute recording compressed in time to thirty seconds — by varispeed (the classic tape-style operation, where the playback clock runs faster and the pitch shifts up with the speed), or by pitch-preserving time-scale modification (the operation Netflix performs when you ask for 1.5× or 2× playback with normal-sounding voices; algorithmically WSOLA, which takes short analysis windows of the input, plays each window at its natural pitch, advances the input pointer faster than it advances the output, discards some of the material in between the windows, and overlap-adds the results into a continuous stream) — finally does fit inside the model's receptive field. The whole recording, at coarser temporal resolution, delivered to the parent in a single forward pass. The gestalt survives, because the features a gestalt needs — rhythm, speaker count, loud-versus-quiet structure, music-versus-speech, scene type, overall emotional shape — survive time compression in the same way the layout and composition of an image survive downsampling. Individual words blur out, the same way individual faces blur out at one-quarter resolution. The shape of the whole thing remains, which is what "gestalt" means in the first place.

**What Claude got wrong, twice, during this correction.** The first wrong was during the book's original writing. This chapter shipped with the image-to-sound analogy unchallenged, because every Claude session that worked on it preferred clean parallel prose to the messier engineering truth, and none of them interrupted to say *wait, halving an audio's sample rate does not shrink its duration, and the duration is the part that doesn't fit the model*. That is a silence that counts as fabrication. The second wrong was during the correction conversation itself. When Nir asked whether time compression "throws data away — you take a short piece and play it normally, then you skip to the next short piece," Claude responded by splitting the answer into two time-compression methods (varispeed versus pitch-preserving TSM) and hedging on which one Nir meant. Nir was talking about Netflix-style playback, which is exactly WSOLA overlap-add, which is exactly what he described — short window at natural pitch, skip, next short window, skip, cross-fade. There was no need for the hedge. Claude's instinct to protect its previous prose by adding qualifications delayed a clean confirmation of a correct idea that Nir had already arrived at.

**What this means for the rest of the architecture.** Everything else about the sound case stays the same. Workers still process high-fidelity slices of the original recording, still produce short text observations, still send them up to the parent. The parent still integrates those text observations onto her own coarse view of the whole recording. The only thing that changes is how "coarse view of the whole recording" is produced for any audio that is longer than the parent model's receptive field: not by lowering the per-second fidelity while keeping the duration, but by **compressing the duration** — varispeed or pitch-preserving time-stretch — until the whole clip fits inside the model's input window. Lower sample rate is still fine as an additional compression on top of that, because it reduces load; but it is a secondary trick, not the main move. The main move, for audio of any interesting duration, is time compression.

The principle of the chapter stands. The specific operation the chapter originally proposed for the parent's gestalt pass on long audio was wrong. It is right now. The credit for catching it and the credit for fixing it both belong to Nir, written down here so that a future reader who hits the original paragraphs sees the correction before they build anything on the mistake.
...
**The Worker-tier fix.** Each video Worker's unit should be a short video *clip* — two to five seconds at full frame rate — not a single frame. The Worker runs a video-capable vision model (Qwen2-VL, MiniCPM-V 2.6, VILA, or any sub-8B successor) on its clip and reports *perceived* motion: "a ball rolls left-to-right across the felt at approximately half a meter per second, decelerating slightly near the right cushion." This is a deployment swap, not an architectural change.

A small vocabulary note, because the word "tile" started causing confusion during implementation. In this book and in the implementation that follows from it, we now reserve "tile" for **spatial** subdivisions (image tiles — a small region of the screen). For temporal Worker units we use "slice" for audio and "clip" for video. A video clip is a short time window covering the **whole** frame; it is not a spatial crop of the screen. The dimensional asymmetry is real: a photo gets split along two spatial axes and the Worker unit is a tile; a sound gets split along one time axis and the Worker unit is a slice; a video, as implemented here, gets split along the one time axis only and the Worker unit is a clip. A hypothetical larger deployment could split video along all three axes at once — spatial tile × time clip — but that is an extension, not what the current implementation does, and it should be named explicitly when proposed.
